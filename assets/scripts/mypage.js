...
function loadReservations() {
    ...
    const userReservations = reservations.filter(r => r.userId === user.username);
...
function displayReservations() {
    ...
    if (reservations.length === 0) {
        reservationList.append('<p class="no-reservation">예매 내역이 없습니다.</p>');
        return;
    }
        ...
        const reservationItem = $(`
            <div class="reservation-item">
                <div class="reservation-info">
                    ...
                    <p>좌석: ${reservation.seats.join(', ')}</p>
                    <p>결제금액: ${reservation.totalPrice.toLocaleString()}원</p>
                    <p>예매일시: ${new Date(reservation.date).toLocaleString()}</p>
                </div>
            </div>
        `);
